...
void HTTPServer::registerURI(const std::string& uri, HTTP::Method method, const RequestHandler& handlerCallback) noexcept
{
    ...
    httpd_uri_t endpoint = {
        .uri = uri.c_str(),
        .method = static_cast<httpd_method_t>(method.value),
        .handler = [](httpd_req_t* request){
            try
            {
                HTTP::Method method = static_cast<HTTP::Method>(static_cast<HTTP::Method::Value>(request->method));
                char contentLengthBuffer[10];
                bool hasContentLength = ESP_OK == httpd_req_get_hdr_value_str(
                    request,
                    "Content-Length",
                    contentLengthBuffer,
                    sizeof(contentLengthBuffer)
                );

                std::string body;
                if(hasContentLength)
                {
                    size_t contentLength = atoi(contentLengthBuffer);
                    std::vector<char> bodyBuffer(contentLength, '\0');
                    httpd_req_recv(request, bodyBuffer.data(), bodyBuffer.size());
                    body.assign(bodyBuffer.begin(), bodyBuffer.end());
                }
                
                UserContext* userContext = static_cast<UserContext*>(request->user_ctx);
                Response response = userContext->handlerCallback(Request(std::string(request->uri), method, body));
...
{
    httpd_unregister_uri_handler(m_server, uri.c_str(), static_cast<httpd_method_t>(method.value));
}
